class BabyTrackerCard extends HTMLElement {
    ...
    render() {
        ...
        // v2.0 Native Entities
        const feedingTimer = this._hass.states['binary_sensor.baby_tracker_allattamento_in_corso']; // Name derived from class
        // Actually unique_id is one thing, entity_id is derived from name. 
        // We named names "Poppate Oggi" etc.
        // Let's use the likely default entity_ids generated by HA
        
        const feedingState = this._hass.states['binary_sensor.baby_tracker_allattamento_in_corso']?.state || 'off';
        
        const pooCount = this._hass.states['sensor.baby_tracker_cacche_oggi']?.state || 0;
        const peeCount = this._hass.states['sensor.baby_tracker_pipi_oggi']?.state || 0;
        const feedingCount = this._hass.states['sensor.baby_tracker_poppate_oggi']?.state || 0;
        ...
        if (btn) {
            btn.onclick = () => {
                // We need a proper service now that input_boolean is gone.
                // For layout purposes, let's assume we add a service.
                const service = feedingState === 'on' ? 'stop_feeding' : 'start_feeding';
                this._hass.callService('baby_tracker', service, {});
            };
        }
    }
